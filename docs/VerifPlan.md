...
1. **Scope**
   ...
   This document establishes the verification plan for the moving average design specified in
...
verification approaches for the moving average design as specified in the requirement 
specification number moving average_req_001(Specification.md).
...
1. **Non-government Documents**
   ...
   Document #: moving average_req_001(Specification.md), Requirement Specification for moving average.
...
* **Test application approach for the moving average**
...
to mimic a moving average in a system environment. The environment will perform the
following transactions at pseudo-random intervals: 
   ...
   | Reset | Directed Test | set reset signal active (high) | All components of moving average enter reset state |
   | Generate samples | Constrained Random | Coverage Driven (full coverage for sample input space) |    *assert* observed data = expected data for each test |
